...
#include <iostream>
using namespace std;
string string_to_binary(char n)
{
	string result;
	do
	{
...
int main() {
	...
	for (int i = 0; i < str.length(); i++) {
		cout << endl;
		for (int j = 0; j < (8 - string_to_binary(str[i]).length()); j++)
		{
			if (str[i] == ' ')
			{
				cout << "00" << string_to_binary(str[i]);
				j++;
			}
			else
				cout << "0" << string_to_binary(str[i]) + " " + str[i];
		}
	}
	HaffTree->CodeDeveloper(HaffTree->getRoot());
	RBTree<char, string> Hf_chart;
	HaffTree->toTree(&Hf_chart, HaffTree->getRoot());
	...
	cout << "\n\nDecode string: ";
	cout << HaffTree->decoder(encode_str);
	cout << "\n\nCompression ratio: ";
